...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeActionFromValues(self, state):
        ...
        if (self.mdp.isTerminal(state)):
            ...
            maxaction = None
            for next in actions:
                val = self.computeQValueFromValues(state, next)
                if max is None:
                    max = val
                    maxaction = next
                elif max < val:
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        "*** YOUR CODE HERE ***"
        states = self.mdp.getStates()
        total_states = len(self.mdp.getStates())
        ...
        for state in states:
            if not self.mdp.isTerminal(state):
                for action in self.mdp.getPossibleActions(state):
                    for next in self.mdp.getTransitionStatesAndProbs(state, action):
                        if next[0] in predecessors:
                            predecessors[next[0]].add(state)
                        else:
                            predecessors[next[0]] = {state}
            ...
            for predecessor in predecessors[state]:
                if not self.mdp.isTerminal(predecessor):
                    ...
                    for action in self.mdp.getPossibleActions(predecessor):
                        ...
                        if max is None or max < value:
                            max = value
                    difff = abs(self.values[predecessor] - max)
                    if difff > self.theta:
                        queue.update(predecessor, -difff)